...
class TestOllamaClient:
    ...
    def test_chat_sends_correct_payload(self, mock_post):
        mock_post.return_value = MagicMock(
            status_code=200,
            json=lambda: {"message": {"content": "ok"}, "total_duration": 0},
        )
        mock_post.return_value.raise_for_status = MagicMock()

        messages = [{"role": "system", "content": "sys"}, {"role": "user", "content": "usr"}]
        self.client.chat(messages, temperature=0.5, num_ctx=16384)

        call_kwargs = mock_post.call_args
        payload = call_kwargs[1]["json"]
        assert payload["model"] == "gemma3:27b-it-qat"
...
class TestParseRcaResponse:
    ...
    def test_json_in_plain_code_fence(self):
        raw = '```\n{"root_cause": "disk full", "category": "resource_exhaustion"}\n```'
        result = self.client.parse_rca_response(raw)
