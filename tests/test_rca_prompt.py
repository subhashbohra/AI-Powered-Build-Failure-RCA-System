...
class TestBuildRcaPrompt:
    ...
    def test_system_prompt_contains_all_categories(self):
        categories = [
            "test_failure", "compilation_error", "dependency_issue",
            "timeout", "resource_exhaustion", "infra_flake", "config_error", "unknown"
        ]
        for cat in categories:
            assert cat in SYSTEM_PROMPT, f"Category '{cat}' not found in system prompt"
    ...
    def test_default_threshold(self):
        ...
        assert "20 minutes" in messages[1]["content"]
