...
Uses GITHUB_API_URL env var to support GitHub Enterprise Server deployments.

Usage:
    python scripts/post_results.py \\
        --repo owner/repo \\
        --sha abc123 \\
        --rca-file rca_output/rca_report.json

Environment variables:
    GITHUB_TOKEN     GitHub PAT with pull-requests:write scope
...
# GitHub API base URL — supports both github.com and GitHub Enterprise Server
...
def find_prs_for_commit(repo: str, sha: str, token: str) -> list[dict]:
    ...
    url = f"{GITHUB_API_URL}/repos/{repo}/commits/{sha}/pulls"
